...
description: >-
  ...
  repo-root `./sandbox` script gives each worktree its own dedicated Postgres
  ...
  database, run `./sandbox`, or query/seed/inspect data in the isolated database.
  Parse whatever they ask into the right sandbox action.
---

# Run the worktree's sandbox

`./sandbox` (a script at the repo root) spins up a **full stack scoped to the
...
- **The script must exist.** If `./sandbox` isn't at the repo root, it hasn't
...
`./sandbox` accepts these subcommands — this is your vocabulary:

| Subcommand         | Does                                                        |
| ------------------ | ---------------------------------------------------------- |
| `./sandbox status` | slug, database, ports, up/down state                       |
| `./sandbox migrate`| ensure the DB exists + apply migrations (no servers)       |
| `./sandbox reset`  | drop + recreate + migrate — **wipes** this worktree's data |
| `./sandbox down`   | **drop** this worktree's database                          |
| `./sandbox up`     | provision + migrate, then run API + web (**blocks**)       |
| `./sandbox psql`   | interactive psql shell (for humans, not you — see §4)      |
...
## 3. Starting the servers (never `./sandbox up` in Bash)

`./sandbox up` runs both dev servers in the **foreground** and blocks until
...
3. Add a config to `.claude/launch.json` (substitute the real port and the
   ...
   {
     "name": "sandbox",
     "runtimeExecutable": "<absolute path to the worktree>/sandbox",
     "runtimeArgs": ["up"],
     "port": <WEB_PORT>
   }
   ```
...
`./sandbox psql` (it opens an interactive shell that will hang the Bash tool).
...
`<DB_NAME>` is the `database` value from `./sandbox status` (e.g.
